...
module.exports = quote = function (args) {
  ...
  if (users.length == 0 && words.length == 0) {
    return 'Try `/quote @name` or `/quote word`';
  }
  ...
  .then(result => {
    ...
    .then(result => {
      ...
      const messages = result.messages.matched.fiter(message => message.channel.id.startsWith('C'));
...
function extractUsersAndWords(message) {
  ...
  const users = [];
  const words = message
    .replace(userRegex, user => users.push(user))
    .replace(/^\s*/, '').replace(/\s*$/, '');
